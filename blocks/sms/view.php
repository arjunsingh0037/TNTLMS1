...
if ($viewpage == 1) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        // Clickatell API call.
            ...
            $sent_count = substr_count($check, "ID");
            if ($sent_count == 0) {
                echo "<script>alert('Message(s) not sent due to an error: $check')</script>";
            } else {
                echo "<script>alert('$sent_count message(s) successfully sent!')</script>";
            }
        }
        // Yutobo API.
