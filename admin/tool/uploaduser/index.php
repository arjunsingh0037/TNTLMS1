...
<script type="text/javascript">
    $(document).ready(function () {
        $('#showtostudent').hide();
        $('#id_settingsheader2').hide();
        $("#usercat").append("<option value='' disabled selected>Select Your Role</option>");
        $("#stream").append("<option value='' disabled selected>Select Stream</option>");
        $("#semester").append("<option value='' disabled selected>Select Semester</option>");
        $("#semyear").append("<option value='' disabled selected>Select Semester Year</option>");           
        $('div[data-groupname="availablefromgroup6"]').hide();
        $('div[data-groupname="availablefromgroup5"]').hide();
        $("#usercat").change(function(){
            var crsid = $(this).val();
            if(crsid == 'student'){
                $('div[data-groupname="availablefromgroup5"]').show();
                $('div[data-groupname="availablefromgroup6"]').hide();
                $('#showtostudent').show('fast');
                $.ajax({
                    url: 'userajax.php',
                    type: 'get',
                    data: {utype:crsid},
                    ...
                    success:function(response){
                        ...
                        $("#program").empty();
                        $("#program").append("<option value='' disabled selected>Select Program</option>");
                        for( var i = 0; i<len; i++){
            ...
            }else if(crsid == 'professor'){
                $('div[data-groupname="availablefromgroup6"]').show();
                $('div[data-groupname="availablefromgroup5"]').hide();
                $('#showtostudent').hide();
            }
        });
        $("#program").change(function(){
            var pname = $('#program').val();
            $.ajax({
                url: 'userajax.php',
                type: 'get',
                data: {pname:pname},
                ...
                success:function(response){
                    ...
                    for( var i = 0; i<len; i++){
                        ...
                        $("#stream").append("<option value='"+id+"'>"+stream+"</option>");
                    }
                }
            });
        });
...
$mform2 = new admin_uploaduser_form2(null, array('columns'=>$filecolumns, 'data'=>array('iid'=>$iid, 'previewrows'=>$previewrows,'usertype'=>$usertype,'program'=>$program,'stream'=>$stream,'semester'=>$semester,'semyear'=>$semyear)));
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        if ($existinguser) {
            ...
            //assign newly created user a role--arjun
            if($user->id){
                $new_tpuser = new stdClass();
                $new_tpuser->userid = $user->id;
                if($_POST['usertype'] == 'student'){
                    role_assign(5, $user->id, SYSCONTEXTID);
                    $new_tpuser->program = $_POST['program'];
                    $new_tpuser->stream = $_POST['stream'];
                    $new_tpuser->semester = $_POST['semester'];
                    $new_tpuser->semyear = $_POST['semyear'];
                    $new_tpuser->roletype = $_POST['usertype'];
                }elseif($_POST['usertype'] == 'professor'){
                    role_assign(3, $user->id, SYSCONTEXTID);
                    $new_tpuser->roletype = $_POST['usertype'];
                }
                $new_tpuser->timecreated = time();
                $new_tpuser->creatorid = $USER->id;
                $DB->insert_record('tp_useruploads',$new_tpuser);  
            }
