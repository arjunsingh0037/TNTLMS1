...
class course_edit_form extends moodleform {
    ...
    function definition() {
                ...
                foreach ($cats as $key => $value) {
                    if($value->idnumber != NULL){
                        ...
                        if($user_cats[1] == $has_account->createdby){
                            $categortobj = $DB->get_record('course_categories',array('id'=>$value->id),'id,name');
                            $ac_categories[$value->id] = $categortobj->name;
                            $cat_custom_id = $value->id;
                        }
                    }
                }
            }
        ...
        $categorycontext = context_coursecat::instance($cat_custom_id);
        ...
        $mform->addElement('text', 'shortname', 'Course code', 'maxlength="100" size="20"');
        $mform->addHelpButton('shortname', 'shortnamecourse');
        //$mform->addRule('shortname', get_string('missingshortname'), 'required', null, 'client');
        $mform->setType('shortname', PARAM_TEXT);
        $crss = $DB->get_records('course');
        $course_count = count($crss) + 2;
        $crs_shortname = 'COD000'.$course_count;
        $mform->setDefault('shortname',$crs_shortname);
        ...
        $radioarray=array();
        $radioarray[] = $mform->createElement('radio', 'subscribed', '', 'Academic', 0, array('id'=>'id_private'));
        $radioarray[] = $mform->createElement('radio', 'subscribed', '', 'Subscribed Industry', 1, array('id'=>'id_public'));
        $mform->addGroup($radioarray, 'radioar', 'Course type', array(' '), false);
        $mform->setDefault('subscribed', 1);

        $mform->addElement('html', '<div class="form-group" id="academic_course" style="display:none">');
        ...
        $mform->addElement('html', '<div class="form-group" id="subscribed_course" style="display:none">');  
        $mform->addElement('html', '</div>');
        // Verify permissions to change course category or keep current.
        if (empty($course->id)) {
            if (has_capability('moodle/course:create', $categorycontext)) {
                //$displaylist = coursecat::make_categories_list('moodle/course:create');
                $mform->addElement('select', 'category', get_string('coursecategory'), $ac_categories);
        ...
        $radioarray2=array();
        $radioarray2[] = $mform->createElement('radio', 'instructionled', '', 'Yes', 1, array());
        $radioarray2[] = $mform->createElement('radio', 'instructionled', '', 'No', 0, array());
        $mform->addGroup($radioarray2, 'radioar2', 'Instruction Led', array(' '), false);

        $radioarray3=array();
        $radioarray3[] = $mform->createElement('radio', 'haselabs', '', 'Yes', 1, array());
        $radioarray3[] = $mform->createElement('radio', 'haselabs', '', 'No', 0, array());
        $mform->addGroup($radioarray3, 'radioar3', 'Has eLabs', array(' '), false);

        $radioarray4=array();
        $radioarray4[] = $mform->createElement('radio', 'trainingtype', '', 'Online', 0, array());
        $radioarray4[] = $mform->createElement('radio', 'trainingtype', '', 'Offline', 1, array());
        $radioarray4[] = $mform->createElement('radio', 'trainingtype', '', 'Online/Offline', 2, array());
        $mform->addGroup($radioarray4, 'radioar4', 'Instruction Led', array(' '), false);

        $radioarray5=array();
        $radioarray5[] = $mform->createElement('radio', 'certification', '', 'Yes', 1, array());
        $radioarray5[] = $mform->createElement('radio', 'certification', '', 'No', 0, array());
        $mform->addGroup($radioarray5, 'radioar5', 'Certification Awarded', array(' '), false);


        // $choices = array();
        // $choices['0'] = get_string('hide');
        // $choices['1'] = get_string('show');
        $mform->addElement('hidden', 'visible', get_string('coursevisibility'));
        ...
        $total_courses= $DB->get_records('course');
        $count_crs = count($total_courses);
        $mform->setDefault('idnumber',$count_crs.'-'.$USER->id);
