...
if ($usernew = $userform->get_data()) {
    ...
    if ($usernew->id == -1) {
        ...
        $usercreated = true;
        //changes by Arjun 
        //assigning roles to newly created roles
        //if mathcing id then assign the same role id

        if($usernew->idnumber == 1){            //account manager created by Superadmin
           role_assign(1, $usernew->id, 1); 
        }elseif($usernew->idnumber == 9){       //partner created by Account Manager 
           //role_assign(9, $usernew->id, 1);
            if($usernew->role == 'partner'){
                role_assign(9, $usernew->id, 1);
                add_partners($usernew); 
            }elseif($usernew->role == 'mentor'){
                role_assign(4, $usernew->id, 1);
                add_mentors($usernew); 
            }
        }elseif($usernew->idnumber == 10){
           role_assign(10, $usernew->id, 1); 
           add_trainingpartners($usernew); 
        }
        //if()
    ...
    if ($user->id == $USER->id) {
        ...
        if(user_has_role_assignment($USER->id, 1, SYSCONTEXTID)) { 
            $returnurl1 = new moodle_url('/user/editadvanced.php', array('id' => -1,'role'=>'partner'));
        }elseif(user_has_role_assignment($USER->id, 9, SYSCONTEXTID) || is_siteadmin()){
            $returnurl1 = new moodle_url('/user/editadvanced.php', array('id' => -1));
        }
        redirect($returnurl1);
    }
    // Never reached..
