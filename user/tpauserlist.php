...
$PAGE->set_url('/user/tpauserlist.php');
require_login();
/*
...
$strpublicprofile = 'Training Partners Lists';
...
echo $OUTPUT->header();
if(user_has_role_assignment($USER->id, 1, SYSCONTEXTID)) { //account manager
    training_partner_list($USER->id,1);
}elseif(user_has_role_assignment($USER->id, 9, SYSCONTEXTID)) { //partner admin
    training_partner_list($USER->id,9);
}elseif(user_has_role_assignment($USER->id, 10, SYSCONTEXTID)) { //partner admin
    training_partner_list($USER->id,10);
}
echo $OUTPUT->footer();
