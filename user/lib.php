...
/**
 * Account Manager adds Partner Admin.
 *
 * @param Object $newuserdata - Newly added user object.
 */
function add_partners($newuserdata) {
    global $CFG,$DB,$OUTPUT,$USER;
    $new_partner = new stdclass();
    $new_partner->userid = $newuserdata->id;
    $new_partner->createdby = $USER->id;
    $new_partner->creator_roleid = '1';
    //permission for partner
    $arr1 = array($newuserdata->usermanageP,$newuserdata->reportmanageP,$newuserdata->newsmanageP,$newuserdata->feedbackP);
    $paaccess = implode(',',$arr1);
    //permission for training partner
    $arr2 = array($newuserdata->usermanageTPA,$newuserdata->badgemanageTPA,$newuserdata->coursemanageTPA,$newuserdata->projectmanageTPA,$newuserdata->taskmanageTPA,$newuserdata->reportmanageTPA,$newuserdata->feedbackTPA);
    $tpaaccess = implode(',',$arr2);
    //permission for hiring company
    ...
    $new_partner->tp_permission = $tpaaccess;
    $new_partner->hc_no = $newuserdata->noofhc;
    $new_partner->hc_permission = $hcaccess;
    $new_partner->stud_no = $newuserdata->noofstu;
    $new_partner->stud_permission = $studaccess;
    $new_partner->prof_no = $newuserdata->noofprof;
    $new_partner->prof_permission = $profaccess;
    $parter_role_assigned = $DB->insert_record('partners',$new_partner);
    if($parter_role_assigned){
        //print_object('Role assigned for partner admin');
    }
}

function add_mentors($newuserdata) {
    global $CFG,$DB,$OUTPUT,$USER;
    $new_mentor = new stdclass();
    $new_mentor->userid = $newuserdata->id;
    $new_mentor->createdby = $USER->id;
    $new_mentor->creator_roleid = '1';
    //permission for mentor
    $arr1 = array($newuserdata->usermanageM,$newuserdata->reportmanageM,$newuserdata->newsmanageM,$newuserdata->feedbackM);
    $maccess = implode(',',$arr1);
   
    $new_mentor->mentor_permission = $maccess;
    $mentor_role_assigned = $DB->insert_record('mentors',$new_mentor);
    if($mentor_role_assigned){
        //print_object('Role assigned for mentor');
    }
}

function add_trainingpartners($newuserdata) {
    global $CFG,$DB,$OUTPUT,$USER;
    $new_trainingpartner = new stdclass();
    $new_trainingpartner->userid = $newuserdata->id;
    $new_trainingpartner->createdby = $USER->id;
    $new_trainingpartner->creator_roleid = '9';
    $new_trainingpartner->stud_no = $newuserdata->maxavailStud;
    $new_trainingpartner->prof_no = $newuserdata->maxavailProf;
    $new_trainingpartner->is_hiringcompany = $newuserdata->ishc;
    $trainingpartner_role_assigned = $DB->insert_record('trainingpartners',$new_trainingpartner);
    if($trainingpartner_role_assigned){
        print_object('Role assigned for TPA');
    }
}
function partnerlist_for_manager($userid){
    global $CFG,$DB;
    $i = 1;
    $user_lists='<div id="exTab3" class=""> 
                    <div class="tab-content ">
                    <div class="tab-pane active">';
                    $user_lists .= '<table id="example1" class="table table-striped table-bordered" style="width:100%">
                    <thead>
                        <tr>
                        <th>S.No</th>
                        <th>Partner Name</th>
                        <th>iTrack UserName</th>
                        <th>Registered On</th>
                        <th>Partner\'s iTrack User Roles(s)</th>
                        <th>Status</th>
                        <th>Edit</th>
                        </tr>
                    </thead><tbody>';
    $partners = $DB->get_records('partners',array('createdby'=>$userid));
    foreach ($partners as $partner) {
    $user = $DB->get_record('user',array('id'=>$partner->userid));
    $user_roles = 'Training Partner<br>Hiring Company<br>Student<br>Professor';
    $user_lists .= '<tr>
                <td>'.$i.'</td>
                ...
                <td><button type="button" class="btn btn-info crslink">Edit</button></td>
                ';
    $user_lists .='</tr>';
    $i++;
    }                        
...
function training_partner_list($userid,$roleid){
    global $CFG,$DB;
    $i= 1;
    if($roleid == 1){
        $user_lists='<div id="exTab4" class=""> 
                        <div class="tab-pane active" id="1">';
                            $user_lists .= '<table id="example1" class="table table-striped table-bordered" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>S.No</th>
                                        <th>Traiining Partner Name</th>
                                        <th>iTrack UserName</th>
                                        <th>Email</th>
                                        <th>Partner Name</th>
                                        <th>Imagine Academy Link</th>
                                    </tr>
                                </thead><tbody>';
                                $partners = $DB->get_records('partners',array('createdby'=>$userid));
                                foreach ($partners as $partner) {
                                    $p_user = $DB->get_record('user',array('id'=>$partner->userid));
                                    $tpas = $DB->get_records('trainingpartners',array('createdby'=>$partner->userid,'is_hiringcompany'=>0));
                                    foreach ($tpas as $tpa) {
                                        $tp_user = $DB->get_record('user',array('id'=>$tpa->userid));
                                        $user_lists .= '<tr>
                                                        <td>'.$i.'</td>
                                                        <td>'.$tp_user->firstname.'</td>
                                                        <td>'.$tp_user->username.'</td>
                                                        <td>'.$tp_user->email.'</td>
                                                        <td>'.$p_user->firstname.'</td>
                                                        <td><button type="button" class="btn btn-info crslink">Enable</button></td>
                                                        ';
                                        $user_lists .='</tr>';
                                        $i++;
                                    }  
        ...
        $user_lists='<div id="exTab3" class=""> 
                        <ul class="nav nav-tabs">
                            <li class="active">
                                <a  href="#1" data-toggle="tab">Training Partners / Colleges</a>
                            </li>
                            <li>
                        ...
                        <div class="tab-content ">
                            ...
                            $user_lists .= '<table id="example1" class="table table-striped table-bordered" style="width:100%">
                                ...
                                $tpas = $DB->get_records('trainingpartners',array('createdby'=>$userid,'is_hiringcompany'=>0));
                                foreach ($tpas as $tpa) {
                                    $tp_user = $DB->get_record('user',array('id'=>$tpa->userid));
                                    $user_lists .= '<tr>
                                                    <td>'.$i.'</td>
        ...
        $user_lists .='</div>
                        <div class="tab-pane" id="2">';
                            $i = 1;
                            $user_lists .= '<table id="example2" class="table table-striped table-bordered" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>S.No</th>
                                ...
                                $hcas = $DB->get_records('trainingpartners',array('createdby'=>$userid,'is_hiringcompany'=>1));
                                foreach ($hcas as $hc) {
                                    $hc_user = $DB->get_record('user',array('id'=>$hc->userid));
                                    $user_lists .= '<tr>
                                                    <td>'.$i.'</td>
                                                    <td>'.$hc_user->firstname.'</td>
                                                    <td>'.$hc_user->username.'</td>
                                                    <td>www.xyz.com</td>
                                                    <td>Arjun Singh</td>
                                                    <td>arjun1@gmail.com</td>
                                                    <td>2342432424</td>
        ...
        $user_lists='<div id="exTab3" class=""> 
                        <ul class="nav nav-tabs">
                            <li class="active">
                                <a  href="#1" data-toggle="tab">Lists of Students / Colleges</a>
                            </li>
                            <li>
                                <a href="#2" data-toggle="tab">List of Professors</a>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content ">
                            ...
                            $user_lists .= '<table id="example1" class="table table-striped table-bordered" style="width:100%">
                                ...
                                $tpas = $DB->get_records('tp_useruploads',array('creatorid'=>$userid,'roletype'=>'student'));
                                foreach ($tpas as $tpa) {
                                    $tp_user = $DB->get_record('user',array('id'=>$tpa->userid));
                                    $stream = $DB->get_record('program_stream',array('id'=>$tpa->stream),'stream');
                                    $user_lists .= '<tr>
                                                    <td>'.$i.'</td>
                                                    <td>'.$tp_user->firstname.'</td>
                                                    <td>'.$tp_user->username.'</td>
                                                    <td>'.$tp_user->email.'</td>
                                                    <td>'.$tp_user->phone1.'</td>
                                                    <td>'.$tpa->program.'</td>
                                                    <td>'.$stream->stream.'</td>
                                                    <td>'.$tpa->semester.'</td>
                                                    <td>'.$tpa->semyear.'</td>
                                                    <td>'.userdate($tpa->timecreated).'</td>
                                                    ';
                                    $user_lists .='</tr>';
                                    $i++;
                                }  
        ...
        $user_lists .='</div>
                        <div class="tab-pane" id="2">';
                            ...
                            $user_lists .= '<table id="example2" class="table table-striped table-bordered" style="width:100%">
                                ...
                                $tpap = $DB->get_records('tp_useruploads',array('creatorid'=>$userid,'roletype'=>'professor'));
                                foreach ($tpap as $tpa) {
                                    $tp_user = $DB->get_record('user',array('id'=>$tpa->userid));
                                    $stream = $DB->get_record('program_stream',array('id'=>$tpa->stream),'stream');
                                    $user_lists .= '<tr>
